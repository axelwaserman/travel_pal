...
tags: [engineering, ml, contract, features, freshness, sla]
status: draft
updated: 2026-08-08
---
...
|---|---|---|---|
| **Booking-time base rate** | Choosing a route/carrier weeks out (P1/P3) | Stale OK (BTS ~3mo lag) | Historical only (§A) |
| **Day-of forecast** | ≥3–6h before departure (P1/P2) | Fresh (≤ minutes) | Historical (§A) + fresh (§B) |
...
**SLA:** rebuilt after each spine top-up ([[ingestion-backfill]] §1.3); staleness bounded by BTS lag; snapshot id stamped for reproducibility.
...
| `reason_codes[]` | top feature attributions (weather / late-aircraft / congestion / carrier) — transparency is the [[differentiation-thesis]] wedge |
| `confidence` | derived from `route_n` + fresh-signal availability |
| `model_version`, `training_window`, `calibration.brier`, `snapshot_id` | auditability for B2B ([[personas]] P4) |
...
- **ML owns:** model family, training from `feat_route_base_rates` + `signals.metar`, **calibration** (reliability/Brier), the artifact interface (load a versioned artifact from SeaweedFS; `predict(feature_vector) → §C`), retraining triggers.
...
- [ ] Calibration bar B2B buyers require (Brier/reliability) — can lakehouse+ML hit it vs Foresight? `#task/ml ⛓ [[staff-ml-engineer]]`
- [ ] Artifact format + versioning convention (SeaweedFS key scheme). `#task/ml`

## Sources
- [[research-summary]] handoff (calibrated `P(delay≥Xh)` + bands), [[personas]] (lead-time), [[differentiation-thesis]] (transparency) — repo vault, accessed 2026-08-08
- Repo: existing marts `agg_route_timeliness`, `agg_*_cancellations`, `fct_flight_performance.sql` — accessed 2026-08-08
